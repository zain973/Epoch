    ...
    <div className="min-h-screen bg-[#FDFBF7] text-zinc-900 font-sans">
      ...
      <main className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="mb-12 text-center md:text-left">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tight" style={{ fontFamily: "Lora, serif" }}>
            Trending Narratives
          </h1>
          <p className="text-zinc-655 mt-2 text-lg max-w-2xl font-light">
          ...
          <div className="space-y-6">
            {articles.map((art, idx) => (
              <article 
                ...
                className="bg-white border border-zinc-200/80 rounded-3xl overflow-hidden hover:shadow-xl hover:border-amber-500/30 transition-all duration-300 cursor-pointer group relative flex flex-col md:flex-row gap-6 p-6 items-center"
              >
                {/* Number Indicator */}
                <div className="absolute top-4 left-6 md:static shrink-0 text-5xl md:text-6xl font-black tracking-tighter text-zinc-100 group-hover:text-amber-500/10 font-serif leading-none w-14 transition-colors">
                  {idx < 9 ? `0${idx + 1}` : idx + 1}
                </div>
                ...
                <div className="flex-1 space-y-3 pt-8 md:pt-0">
                  <div className="flex items-center gap-2">
                    {art.tags?.slice(0, 2).map(tag => (
                      <span key={tag} className="text-[9px] font-extrabold uppercase tracking-widest text-amber-600 bg-amber-500/5 px-2 py-0.5 rounded">
                        {tag}
                      </span>
                    ))}
                  </div>

                  <h3 className="text-xl font-bold leading-snug group-hover:text-amber-600 transition-colors text-zinc-950" style={{ fontFamily: "Lora, serif" }}>
                    {art.title}
                  </h3>

                  <p className="text-zinc-600 text-xs md:text-sm line-clamp-2 leading-relaxed font-light">
                    {art.content.slice(0, 150)}…
                  </p>

                  {/* Analytics Bar */}
                  <div className="flex flex-wrap items-center gap-5 text-[10px] font-semibold text-zinc-400 pt-1.5 border-t border-zinc-100">
                    <span className="text-zinc-700 font-semibold">{art.author}</span>
                    <span className="w-1.5 h-1.5 bg-zinc-200 rounded-full" />
                    <span className="flex items-center gap-1"><Eye className="w-3.5 h-3.5 text-zinc-300" /> {art.views} views</span>
                    <span className="flex items-center gap-1"><Heart className="w-3.5 h-3.5 text-zinc-300" /> {art.likes} likes</span>
                    <span className="flex items-center gap-1"><Share2 className="w-3.5 h-3.5 text-zinc-300" /> {art.shares} shares</span>
                  </div>
                </div>
              </article>
            ))}
